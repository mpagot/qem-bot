...
def test_incidents_printable():
    ...
    test_config = {}
    test_config["FLAVOR"] = {}
    inc = Incidents(product="hello", settings=None, config=test_config, extrasettings=None)
...
def test_incidents_call_with_incidents():
    ...
    inc = Incidents(product="", settings={'VERSION': '', 'DISTRI':None}, config=test_config, extrasettings=None)
...
def request_mock(monkeypatch):
    ...
    monkeypatch.setattr(
        "openqabot.types.incidents.requests.get",
        mock_get
    )
...
def test_incidents_call_with_channels(request_mock):
    ...
    inc = Incidents(product="", settings={'VERSION': '', 'DISTRI':None}, config=test_config, extrasettings=set())
    res = inc(incidents=[MyIncident()], token={}, ci_url="", ignore_onetime=False)
    assert len(res) == 1
...
def test_incidents_call_public_cloud_pint_query(request_mock, monkeypatch):
    ...
    inc = Incidents(product="", settings={'VERSION': '', 'DISTRI':None, 'PUBLIC_CLOUD_PINT_QUERY': None}, config=test_config, extrasettings=set())
    res = inc(incidents=[MyIncident()], token={}, ci_url="", ignore_onetime=False)
    assert len(res) == 1
